library(e1071)
set.seed(2)
# NOTE: TOWARDS THE END IT DOESNT GIVE THE BEST MODEL. JUST KNOW TO CHOOOSE THE ONE WITH THE BEST COST
#  FROM THE TNUE
...
# What if we instead used a smaller value of the
# cost parameter?
# now cost = 0.1
...
# Now that a smaller value of the cost (cost
# =0.1) parameter is being used, we obtain a
# larger number of support vectors, because
# the margin is now wider.

# Unfortunately, the svm() function does not
# explicitly output the coefficients of the linear
# decision boundary obtained when the support
# vector classifier is fit, nor does it output the
# width of the margin.
...
# • By default, tune() performs ten-fold cross-
#   validation on a set of models of interest. In
# order to use this function, we pass in relevant
# information about the set of models that are
# under consideration.
# • The following command indicates that we
# want to compare SVMs with a linear kernel,
# using a range of values of the cost
# parameter. 
...
summary(tune.out)
# We see that cost=0.1 (1 for me) results in the lowest cross-validation error rate.
...
# There's a lot more for nov 11th lab
